...
public:
    ...
    PlaylistRowMenuItem(int8_t slot) : MenuItem("Slot  0", true, false), my_slot(slot) {
        ...
        IF_MENU_PERF_PARTIAL_UPDATES(this->add_redraw_policy(REDRAW_ON_BAR);)   // to update highlight when current slot's section changes TODO: should also track changes to the current song section, etc
    }
